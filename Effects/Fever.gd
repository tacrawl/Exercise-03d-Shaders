...
func start_fever():
	fever()
	$Timer.start()
	var fever_inidcator = get_node_or_null("/res/GAme/UI/HUD/Fever")
	if fever_inidcator != null:
		fever_inidcator.use_parent_material = false
...
func _on_Timer_timeout():
	if Global.feverish:
		...
		var fever_inidcator = get_node_or_null("/res/Game/UI/HUD/Fever")
		if fever_inidcator != null:
			fever_inidcator.use_parent_material = true
		var background = get_node_or_null("/root/Game/Background")
		if background != null:
